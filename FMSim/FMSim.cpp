...
int main()
{
	...
	try
	{
		...
		for (int matchIndex = 0; matchIndex < simulationCount; ++matchIndex)
		{
			...
			Team homeTeam("Istanbul Stars", createPlayers("Home", 72, 68, 76), TacticType::Possession, DefenseTacticType::Pressing);
			Team awayTeam("Ankara Wolves", createPlayers("Away", 69, 70, 74), TacticType::CounterAttack, DefenseTacticType::ZonalMarking);
		...
		summaryFile << "Takimlar: Istanbul Stars vs Ankara Wolves" << std::endl << std::endl;
		...
		for (std::size_t index = 0; index < matchResults.size(); ++index)
		{
			...
			summaryFile << "[" << (index + 1) << "] Istanbul Stars "
				<< result.homeScore << " - " << result.awayScore
				<< " Ankara Wolves" << std::endl;
		}

		summaryFile << std::endl;
		summaryFile << "Istanbul Stars galibiyet: " << homeWins << std::endl;
		summaryFile << "Ankara Wolves galibiyet: " << awayWins << std::endl;
		summaryFile << "Beraberlik: " << draws << std::endl;
		summaryFile << "Toplam gol | Istanbul Stars: " << totalHomeGoals
			<< " | Ankara Wolves: " << totalAwayGoals << std::endl;
		summaryFile << "Mac basi gol | Istanbul Stars: " << std::fixed << std::setprecision(2)
